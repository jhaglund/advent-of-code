...
function part1(data){
  ...
  function lineCheck(i, match){
    ...
    const len = String(match).length + 2;
    const testLine = data[ prevIndex ].substring(start, start+len)
        + data[ i ].substring(start, start+len)
        + data[ nextIndex ].substring(start, start+len);
    // return true if there is a match
    return (testLine.match(matchSymbols) !== null)
  }
  data.forEach((row, i)=>{
    let match;
    while((match = matchNumbers.exec(row)) !== null){
      // for each match, look around for characters. add to sum if found
      if( lineCheck(i, match) ){
        sum += parseInt(match);
      }
    }
  });
