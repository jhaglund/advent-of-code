...
const reversedNumbersRegexp = /enin|thgie|neves|xis|evif|ruof|eerht|owt|eno/;

function sumFn(sum, val){
  let twoDigit = '';
  const origVal = val;
  let matches = val.match(numbersRegexp);
  if(matches != null){
    matches.forEach((text) => {
      let number = numbers.indexOf(text);
  ...
  if(matches != null){
    matches.forEach((text) => {
      let number = revNumbers.indexOf(text);
